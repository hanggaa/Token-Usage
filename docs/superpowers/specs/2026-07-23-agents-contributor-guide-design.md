...
6. **Security & Agent Instructions** — preserve the local-only privacy model, keep source histories read-only, never access authentication files, maintain exact/estimated/partial lower-bound/unavailable distinctions, protect unrelated working-tree changes, and do not commit generated or local artifacts such as `node_modules/`, `dist/`, `coverage/`, `*.vsix`, `.superpowers/`, `graphify-out/`, or `.DS_Store`.
